...
//                 (换 code 在 /api/auth/wechat/callback 用 lib/auth/wechat-mp.ts 完成),
//                 这里只负责认人(upsert users)+ 建会话,不直接调微信 API。
//   "phone-otp" — 手机号 + 验证码;authorize 校验 OTP(lib/auth/otp.ts)后 upsert。
import NextAuth from "next-auth";
import Credentials from "next-auth/providers/credentials";
import { upsertWechatUser, upsertPhoneUser } from "@/lib/auth/users";
...
export const { handlers, signIn, signOut, auth } = NextAuth({
  ...
  providers: [
    ...
    Credentials({
      ...
      authorize: async (cred) => {
        const phone = typeof cred?.phone === "string" ? cred.phone : "";
        const code = typeof cred?.code === "string" ? cred.code : "";
        if (!phone || !code) return null;
        const ok = await verifyOtp(phone, code);
        if (!ok) return null;
        const id = await upsertPhoneUser(phone);
        return { id };
      },
    }),
  ],
